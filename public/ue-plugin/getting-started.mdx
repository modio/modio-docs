...
slug: /ue-plugin/getting-started
sidebar_position: 1
---
...
When you are ready to initialize the plugin for the current user, you'll need to call [K2_InitializeAsync](#K2_InitializeAsync), passing in an instance of `FModioInitializeOptions`, and a delegate so you know when the plugin is initialized correctly. 
...
1. Call [K2_AuthenticateUserExternalAsync](#K2_AuthenticateUserExternalAsync) , passing in any provider-specific parameters, setting `AuthenticationParams::bUserHasAcceptedTerms` to false, and indicating which authentication provider you wish to use
