...
You should either enable the [Configuration](setup/#plugin-configuration) setting to allow the plugin to manage a background thread for you which will call [RunPendingHandlers](https://docs.mod.io/unrealref/#RunPendingHandlers), or else invoke it on Tick on the game thread yourself. 
...
If you have configured your game’s API Key, ID and environment via the Plugin Settings window as in [Configuration](setup/#plugin-configuration), you can use the convenience function [Get Project Initialize Options For Session Id](https://docs.mod.io/unrealref/#GetProjectInitializeOptionsForSessionId):
