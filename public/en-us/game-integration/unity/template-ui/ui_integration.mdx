---
id: ui-integration
title: Integrating the Template UI
slug: /unity/template-ui/ui-integration/
custom_edit_url: https://github.com/modio/modio-docs-internal/tree/main/public/en-us/game-integration/unity/template-ui/ui_integration.mdx
---

![template ui overview](img/integration_unity.png)

The Template UI has two key goals; provide a series of components that are easier to integrate into all designs, and a robust UI suitable for dropping into your project as is or with as much customization as you'd like.

:::important
The Component UI relies on the config file that is configured during the following setup instructions on the main [Unity Modio readme](https://docs.mod.io/unity/setup/). Please ensure you have completed all of those steps before proceeding.
:::  
...
We use monobehaviours like ModioUIMod to act as containers for ``ModioUI.Data.Mod``, which holds all the details you may wish to know about the mod. Children of that gameobject will add ``ModioUIModProperties`` monoBehaviours, which have properties added to update UI or add listeners to buttons (such as to subscribe or get ratings)
...
- **ModioUIUser** and **ModioUIUserProperties**, which has details about either the logged in local user or the author of a mod.
    - If you want to display the author of a mod, use a ``ModProperties`` with the Creator property, pointed at a ModioUIUser on the same GameObject. You can then use ModioUIUserProperties to display details like the name or icon
