...
2. Confirm `your_project` uses CMake 3.20 or later
   ...
   ```
3. Specify the target platform with options: GDK_XBOX, GDK_XSX or GDK_WIN

   ```cmake
   set (MODIO_PLATFORM GDK_XBOX)
   set (MODIO_GDK_DIR "Path to XBox GDK installation")
   ```
...
To simplify the CMake configuration, the folder platform/gdk/cmake has the file "CMakePresets.json" with the base configuration required to guide the mod.io SDK compilation using the GDK. If you copy that file to the folder root, it can be used as follows:

```cmake
cmake --preset gdk-xbox
```
