...
To mute a user, call [`MuteUserAsync`](/unrealref/#K2_MuteUserAsync) with the corresponding `ModioUserID` and a callback.
...
To perform the inverse operation call [`UnmuteUserAsync`](/unrealref/#K2_UnmuteUserAsync) with the corresponding `ModioUserID` and a callback.
...
[`GetMutedUsersAsync`](/unrealref/#K2_GetMutedUsersAsync) returns a list of users previously muted by an authenticated user. 
