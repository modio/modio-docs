...
Users must accept the mod.io Terms of Service before using any mod.io services.  The Terms of Service can be fetched from mod.io using [`GetTermsOfUseAsync`](/unrealref/#K2_GetTermsOfUseAsync).  Before attempting authentication, ensure these terms are displayed to and accepted by the user.  
...
Request a one-time authentication code by calling [`RequestEmailAuthCodeAsync`](/unrealref/#K2_RequestEmailAuthCodeAsync).  Pass in a user-provided email address as the first argument.
...
To finalize the user's authentication, submit the code they have entered to mod.io by calling [`AuthenticateUserEmailAsync`](/unrealref/#K2_AuthenticateUserEmailAsync).  Pass the user's code in as the first argument.
...
The mod.io Unreal Engine plugin features single sign-on (SSO) authentication from a number of [external providers](/unrealref/#_emodioauthenticationprovider). These include:
...
Each platform has their own requirements and prerequisites for performing SSO.  Platform-specific authentication can be found in the respective [platform documentation](/platforms/).  SSO is performed by calling [`AuthenticateUserExternalAsync`](/unrealref/#K2_AuthenticateUserExternalAsync).  This function takes a [`ModioAuthenticationParams`](/unrealref/#_modioauthenticationparams) containing provider-specific parameters, and a [`ModioAuthenticationProvider`](/unrealref/#_emodioauthenticationprovider) indicating the provider you're using.   

[Unreal Engine's Online Subsystem](#using-the-unreal-online-subsystem) provides a convenient way to get a token for `ModioAuthenticationParams.AuthToken`. [`GetDefaultAuthProviderForCurrentPlatform`](/unrealref/#GetDefaultAuthProviderForCurrentPlatform) can be used to get the default provider for the current platform. 
...
By default, tokens issued via email token exchange have a lifetime of 1 year. You can verify that a user has been successfully authenticated with [`VerifyUserAuthenticationAsync`](/unrealref/#K2_VerifyUserAuthenticationAsync). 
