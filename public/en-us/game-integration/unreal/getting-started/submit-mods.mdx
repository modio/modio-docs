...
To submit a mod, you must first create a mod handle using [`GetModCreationHandle`](https://docs.mod.io/unrealref/#K2_GetModCreationHandle), and use that handle when calling [`SubmitNewModAsync`](https://docs.mod.io/unrealref/#K2_SubmitNewModAsync). Note that the newly created mod will remain hidden until a mod file is added in the next step.
...
Once you have successfully submitted a mod, you can submit a mod file for that mod using [`SubmitNewModFileForMod`](https://docs.mod.io/unrealref/#K2_SubmitNewModFileForMod). When you submit a mod file, you pass a [`ModioCreateModFileParams`](https://docs.mod.io/unrealref/#_modiocreatemodfileparams) containing the directory containing all the files that you want to submit. The plugin will compress this folder into a .zip file and upload it as the active version of the mod. 
