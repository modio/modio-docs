...
| ```___UGCNAME___```   | The name given to the item (synonymous with ```___MODNAME___``` for mod templates)   | 

## What is in a Template?
Each template is a zip file that contains two parts: the files to copy and the template descriptor.

When a user creates content with a template the files contained in the zip file are copied in to the Mod directory in the same directory structure. This structure should mirror the expected folder structure for Unreal plugins and begin with ```\Content\```. The ```\Resources\``` folder contains meta information that is useful for the UI and Mod templates but will not get copied for Item templates.
...
| --------          | ----          | ------- | --------   | ----------- |
| Version           | int           | 1       | Yes        | The version number for this template file |
...
| TemplateVersion   | String        | 1       | Optional   | Version string to display to users |
| Description       | String        | 1       | Optional   | Description of what this template creates |
| Category          | String        | 1       | Optional   | Used for filtering in the UI |
...
4. Open the UGC Template Export tool. This can be accessed via:
    * The Editor Toolbar
    * File Menu
