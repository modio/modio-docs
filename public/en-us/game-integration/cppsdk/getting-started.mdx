...
:::note
A call to [ClearUserDataAsync](/cppsdk/refdocs#clearuserdataasync) removes the authenticated user from the local device. Any installed content is marked for uninstallation from local storage if no other Local Profiles contain authenticated users with active subscriptions to it.
:::
...
For more details on the error codes and how to inspect their values, please see [SDK quick-start: Error Handling](#sdk-quick-start-error-handling) and [ErrorCode](/cppsdk/refdocs#ErrorCode).
...
As mentioned in [the quick-start section on initialization](#sdk-quick-start-initialization-and-teardown), the SDK supports [RunPendingHandlers](/cppsdk/refdocs#runpendinghandlers) being run on a secondary thread that already exists or a dedicated background thread you create specifically to peform SDK work.
...
If you are supporting the purchase of token packs on platform storefronts, entitlement refreshing is the method by which those token packs are consumed and converted into mod.io virtual tokens for users to purchase UGC with.
