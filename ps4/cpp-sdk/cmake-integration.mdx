...
2. Confirm `your_project` uses CMake 3.20 or later
   ...
   ```
3. Specify the target platform with ORBIS

   ```cmake
   set (MODIO_PLATFORM ORBIS)
   ```
...
To simplify the CMake configuration, the folder platform/orbis/cmake has the file "CMakePresets.json" with the base configuration required to guide the mod.io SDK compilation using the PS4&trade; SDK. If you copy that file to the folder root, it can be used as follows:
