...
1. **Detection:** When you initiate a build (e.g compile for Android or Windows), UBT checks for an environment variable called `UE_SDKS_ROOT`.
...
:::note
Simply copying installed SDKs (e.g. from `C:\Program Files`) will **not** work, as they often rely on static registry keys.
:::

The general workflow for setting these up is as follows:

1. **Source:** Download the official SDK installer from the platform holder's website (e.g Sony DevNet, Microsoft GDK, Nintendo Developer Portal).
2. **Extraction:** Run the platform-specific generation script to create the "portable" file structure.
...
3. **Placement:** Move the output into the correct AutoSDK path: `AutoSDK/Host[OS]/[Platform]/[Version]`.
4. **Turnkey Configuration:** Update your project's `TurnkeyManifest.xml` (Located in MyProject/Extras/Turnkey/TurnkeyManifest.xml) to whitelist the new SDK version. This ensures the Unreal Turnkey system recognizes the new version as valid.

Commit this folder to Source Control.

### 2. Client setup

1. Sync the AutoSDK folder from source control.
2. Set a system environment variable named `UE_SDKS_ROOT` pointing to that folder.
   - **Example:** `UE_SDKS_ROOT=D:\P4\MyProject\AutoSDK`
3. Restart the Unreal Editor or generate project files to refresh the environment detection.

### 3. Building

No further action is required. When a user selects **Package Project** or builds via Visual Studio, Unreal Engine detects the SDK in `UE_SDKS_ROOT` and uses it automatically.
