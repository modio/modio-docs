...
The [ModioUGC](/unreal/modio-ugc/overview/) feature is composed of two core components: `UUGCSubsystem`, which handles UGC discovery and loading, and `IUGCProvider`, which interacts with UGC providers to determine availability and locate UGC packages.
