...
```
### Set up
We recommend that you create subclasses of `UMutator` and `UMutatorSubsystem` in your project. For the system to pick them up you will also need to add the following to your projects `*.Build.cs` file:

```cs
PublicDefinitions.Add("MUTATOR_CLASS=<Your Mutator Class Name>");
PublicDefinitions.Add("MUTATOR_SUBSYSTEM_CLASS=<Your Mutator Subsystem Class Name>");
```
...
It is also possible to add default values for the parameters.

Example:
```cpp
DEFINE_MUTATOR(EnemyWaveEnded, int32, WaveNumber = 1)
```
