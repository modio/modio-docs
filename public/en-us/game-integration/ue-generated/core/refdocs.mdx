...
### ModioModCollectionInfoListLibrary 
...
### ModioMultiplayerSubsystem 

`ModioMultiplayerSubsystem` is a thin wrapper around the mod.io  Multiplayer SDK Library, wrapping all the functions available in the SDK's public header `modio/detail/ModioSDKMultiplayerLibrary.h`. This subsystem also converts mod.io Multiplayer SDK types to unreal-friendly types and caches some expensive operations.
...
Adds the provided Mods to the Server's list of client mods. Intended to be called on the server. Once complete it is expected to propagate the server's list of client mods back to all clients to ensure syncronization
:::warning[Experimental Feature]
This function is part of an experimental feature and is subject to change. 
:::

![nd_img_ModioMultiplayerSubsystem_K2_RegisterClientModsWithServerAsync.png](img/refdocs/nd_img_ModioMultiplayerSubsystem_K2_RegisterClientModsWithServerAsync.png)

```cpp
void K2_RegisterClientModsWithServerAsync(TArray<FModioModID> ModIDs, FAddClientModsDelegate Callback)
```
...
| | |
|-|-|
|`Target`|`ModioMultiplayerSubsystem`|
|`ModIDs`|a list of mods that the joining client has|
|`Callback`|callback upon completion of adding the mods to the Server's list.|
</RefTable>
...
Gets and/or updates all of the indicated mods for this server, assuming you have provided a valid S2S token. Is called automatically by InitialalizeModioServerAsync() depending on UpdateModsOnInit state.
:::warning[Experimental Feature]
This function is part of an experimental feature and is subject to change. 
:::

![nd_img_ModioMultiplayerSubsystem_K2_InstallOrUpdateServerModsAsync.png](img/refdocs/nd_img_ModioMultiplayerSubsystem_K2_InstallOrUpdateServerModsAsync.png)

```cpp
void K2_InstallOrUpdateServerModsAsync(TArray<FModioModID> Mods, FOnErrorOnlyDelegate Callback)
```
...
|-|-|
|`Target`|`ModioMultiplayerSubsystem`|
|`Mods`|the mods to get or update|
|`Callback`|callback executed upon completion of all mods|
</RefTable>
...
Initializes the SDK in the context of a dedicated server. Loads the state of all mods installed on the system. Loads or gets all mods indicated in the initialization param, optionally updating to the latest versions.
:::warning[Experimental Feature]
This function is part of an experimental feature and is subject to change. 
:::

![nd_img_ModioMultiplayerSubsystem_K2_InitializeServerAsync.png](img/refdocs/nd_img_ModioMultiplayerSubsystem_K2_InitializeServerAsync.png)

```cpp
void K2_InitializeServerAsync(FModioServerInitializeOptions ServerOptions, FOnErrorOnlyDelegate OnInitComplete)
```
...
Returns the full list of mods that have been registered with the server via AddClientModsToServerAsync
:::warning[Experimental Feature]
This function is part of an experimental feature and is subject to change. 
:::

![nd_img_ModioMultiplayerSubsystem_K2_GetRegisteredClientMods.png](img/refdocs/nd_img_ModioMultiplayerSubsystem_K2_GetRegisteredClientMods.png)

```cpp
TSet<FModioModID> K2_GetRegisteredClientMods()
```
...
##### Returns
A TSet of ModIDs representing the Mods registered with the server.
...
Clears the server's list of client mods. Intended only to be called when the Server can safely flush its list of client mods, if you want to minimize the number of mods you send to clients/remove mods no longer needed.
:::warning[Experimental Feature]
This function is part of an experimental feature and is subject to change. 
:::

![nd_img_ModioMultiplayerSubsystem_K2_ClearRegisteredClientMods.png](img/refdocs/nd_img_ModioMultiplayerSubsystem_K2_ClearRegisteredClientMods.png)

```cpp
void K2_ClearRegisteredClientMods()
```
...
### ModioPortalInterface 

Interface for objects that provide portal/platform specific implementations
...
#### Should Show SKUInformation



![nd_img_ModioPortalInterface_ShouldShowSKUInformation.png](img/refdocs/nd_img_ModioPortalInterface_ShouldShowSKUInformation.png)

```cpp
bool ShouldShowSKUInformation()
```
...
| | |
|-|-|
|`Target`|`ModioPortalInterface`|
</RefTable>



##### Returns
true if the UI should show SKU information, false otherwise
...
![nd_img_ModioPortalInterface_SanitizeFilePath.png](img/refdocs/nd_img_ModioPortalInterface_SanitizeFilePath.png)
...
![nd_img_ModioPortalInterface_RequestStorePurchase.png](img/refdocs/nd_img_ModioPortalInterface_RequestStorePurchase.png)

```cpp
bool RequestStorePurchase(FString SKU, FStorePurchaseDelegate Callback)
```
...
![nd_img_ModioPortalInterface_RequestSKUMappings.png](img/refdocs/nd_img_ModioPortalInterface_RequestSKUMappings.png)

```cpp
void RequestSKUMappings(FSKUMappingsRequestedDelegate Callback)
```
...
![nd_img_ModioPortalInterface_RequestOpenStore.png](img/refdocs/nd_img_ModioPortalInterface_RequestOpenStore.png)

```cpp
EModioOpenStoreResult RequestOpenStore(FString SKU, FStoreClosedDelegate Callback)
```
...
#### Request Entitlement Params



![nd_img_ModioPortalInterface_RequestEntitlementParams.png](img/refdocs/nd_img_ModioPortalInterface_RequestEntitlementParams.png)

```cpp
void RequestEntitlementParams(TMap<FString,FString> Params, FEntitlementParamsRequestedDelegate Callback)
```
...
|-|-|
|`Target`|`ModioPortalInterface`|
|`Params`|can provide additional parameters for login|
|`Callback`|will be called with the entitlement parameters|
</RefTable>
...
![nd_img_ModioPortalInterface_RequestAuthToken.png](img/refdocs/nd_img_ModioPortalInterface_RequestAuthToken.png)

```cpp
void RequestAuthToken(TMap<FString,FString> Params, FAuthTokenRequestedDelegate Callback)
```
...
|`Target`|`ModioPortalInterface`|
|`Params`|can provide additional parameters for login|
|`Callback`|will be called with the auth token|
</RefTable>
...
#### Prepare Filesystem to Use Path


![nd_img_ModioPortalInterface_PrepareFilesystemToUsePath.png](img/refdocs/nd_img_ModioPortalInterface_PrepareFilesystemToUsePath.png)

```cpp
bool PrepareFilesystemToUsePath(FString UgcPath)
```
...
#### Notify Store Browsing



![nd_img_ModioPortalInterface_NotifyStoreBrowsing.png](img/refdocs/nd_img_ModioPortalInterface_NotifyStoreBrowsing.png)

```cpp
void NotifyStoreBrowsing(bool bEnabled)
```
...
|-|-|
|`Target`|`ModioPortalInterface`|
|`bEnabled`|whether the store is being browsed or not|
</RefTable>
...
![nd_img_ModioPortalInterface_InitializePlatform.png](img/refdocs/nd_img_ModioPortalInterface_InitializePlatform.png)
...
#### Get Extended Initialization Params


![nd_img_ModioPortalInterface_GetExtendedInitializationParams.png](img/refdocs/nd_img_ModioPortalInterface_GetExtendedInitializationParams.png)
...
#### Get Extended Auth Params


![nd_img_ModioPortalInterface_GetExtendedAuthParams.png](img/refdocs/nd_img_ModioPortalInterface_GetExtendedAuthParams.png)
...
##### Returns
`TMap<FModioModID, FModioModCollectionEntry>` providing information about the subscribed mods
...
Sends a request to the mod.io server to remove the specified mod collection to the user's list of subscriptions.

![nd_img_ModioSubsystem_K2_UnsubscribeFromModCollectionAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_UnsubscribeFromModCollectionAsync.png)

```cpp
void K2_UnsubscribeFromModCollectionAsync(FModioModCollectionID ModCollectionToUnsubscribeFrom, FOnErrorOnlyDelegate Callback)
```
...
|`Callback`|Callback providing a status code to indicate if the unsubscription was successful.|
</RefTable>
...
Unfollows the given user for the currently authenticated user.

![nd_img_ModioSubsystem_K2_UnfollowUserAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_UnfollowUserAsync.png)

```cpp
void K2_UnfollowUserAsync(FModioUserID UserId, FOnErrorOnlyDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`UserId`|The UserID of the user that should be unfollowed.|
|`Callback`|a callback invoked upon completion, indicating if the request was successful.|
</RefTable>
...
|-|-|
|GenericError::SDKNotInitialized|SDK not initialized
|UserDataError::InvalidUser|User not authenticated
...
Sends a request to the mod.io server to remove the specified mod collection to the user's list of followed collections.

![nd_img_ModioSubsystem_K2_UnfollowModCollectionAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_UnfollowModCollectionAsync.png)

```cpp
void K2_UnfollowModCollectionAsync(FModioModCollectionID ModCollectionToUnfollow, FOnErrorOnlyDelegate Callback)
```
...
|`Callback`|Callback invoked when the unfollow request is completed.|
</RefTable>
...
Sends a request to the mod.io server to add the specified mod collection to the user's list of subscriptions.

![nd_img_ModioSubsystem_K2_SubscribeToModCollectionAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_SubscribeToModCollectionAsync.png)

```cpp
void K2_SubscribeToModCollectionAsync(FModioModCollectionID ModCollectionToSubscribeTo, FOnErrorOnlyDelegate Callback)
```
...
|`Callback`|Callback providing a status code to indicate if the subscription was successful.|
</RefTable>
...
![nd_img_ModioSubsystem_K2_SubmitNewSourceFileForMod.png](img/refdocs/nd_img_ModioSubsystem_K2_SubmitNewSourceFileForMod.png)
...
Submits a rating for a mod collection on behalf of the current user. Submit a neutral rating to effectively clear a rating already submitted by a user. Submitting other values will overwrite any existing rating submitted by this user.

![nd_img_ModioSubsystem_K2_SubmitModCollectionRatingAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_SubmitModCollectionRatingAsync.png)

```cpp
void K2_SubmitModCollectionRatingAsync(FModioModCollectionID ModCollectionId, EModioRating Rating, FOnErrorOnlyDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`ModCollectionId`|The mod collection the user is rating|
|`Rating`|The rating the user wishes to submit|
|`Callback`|Callback providing a status code indicating success or failure of the rating submission|
</RefTable>
...
![nd_img_ModioSubsystem_K2_PurchaseModWithEntitlementAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_PurchaseModWithEntitlementAsync.png)

```cpp
void K2_PurchaseModWithEntitlementAsync(FModioModID ModID, FModioEntitlementParams EntitlementParams, FOnPurchaseModDelegate Callback)
```
...
|`EntitlementParams`|Portal-specific additional parameters|
...
This function requires the **Metrics** premium feature. Sign up to one of our [advanced service tiers](https://mod.io/pricing) to activate it, or contact us at developers@mod.io for more information.
:::
...
![nd_img_ModioSubsystem_K2_ListUserFollowedModCollectionsAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_ListUserFollowedModCollectionsAsync.png)

```cpp
void K2_ListUserFollowedModCollectionsAsync(FModioFilterParams Filter, FOnListFollowedModCollectionsDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`Filter`|`FModioFilterParams` object containing any filters that should be applied to the query.|
|`Callback`|Callback providing a status code and an optional `FModioModCollectionInfoList` object with the mod collection's extended information.|
</RefTable>
...
![nd_img_ModioSubsystem_K2_ListModCollectionsAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_ListModCollectionsAsync.png)

```cpp
void K2_ListModCollectionsAsync(FModioFilterParams Filter, FOnListModCollectionsDelegate Callback)
```
...
|`Target`|`ModioSubsystem`|
|`Filter`|[`ModioFilterParams`](#modiofilterparams) object containing any filters that should be applied to the query.|
|`Callback`|Callback invoked with a status code and an optional `FModioModCollectionInfoList` providing mod collection profiles.|
</RefTable>
...
![nd_img_ModioSubsystem_K2_GetUserRatingsAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetUserRatingsAsync.png)

```cpp
void K2_GetUserRatingsAsync(FOnGetUserModRatingsDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`Callback`|Callback invoked with a status code and an optional UserModRatingList providing ratings for each|
</RefTable>
...
![nd_img_ModioSubsystem_K2_GetUserFollowingAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetUserFollowingAsync.png)

```cpp
void K2_GetUserFollowingAsync(FModioUserID UserId, FOnGetFollowsDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`UserId`|the UserID of the user you are querying.|
|`Callback`|a Callback invoked with a status code and optional User List containing the list of followers|
</RefTable>
...
![nd_img_ModioSubsystem_K2_GetUserFollowersAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetUserFollowersAsync.png)

```cpp
void K2_GetUserFollowersAsync(FModioUserID UserId, FOnGetFollowsDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`UserId`|the UserID of the user who's followers you are getting|
|`Callback`|a Callback invoked with a status code and optional User List containing the list of followers|
</RefTable>
...
![nd_img_ModioSubsystem_K2_GetModCollectionModsAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetModCollectionModsAsync.png)

```cpp
void K2_GetModCollectionModsAsync(FModioModCollectionID ModCollectionId, FOnGetModCollectionModsDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`ModCollectionId`|Mod Collection ID of the mod collection to fetch data.|
|`Callback`|Callback providing a status code and an optional `FModioModCollectionInfoList` object with the mod collection's extended information.|
</RefTable>
...
#### GetModCollectionMediaAsync (Logo)

Downloads the logo for the specified mod collection. Will use existing file if it is already present on disk.

![nd_img_ModioSubsystem_K2_GetModCollectionLogoAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetModCollectionLogoAsync.png)

```cpp
void K2_GetModCollectionLogoAsync(FModioModCollectionID ModCollectionId, EModioLogoSize LogoSize, FOnGetModCollectionMediaDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`ModCollectionId`|Mod Collection ID for use in logo retrieval.|
|`LogoSize`|Parameter indicating the size of logo that's required.|
|`Callback`|Callback providing a status code and an optional path object pointing to the location of the downloaded image|
</RefTable>
...
![nd_img_ModioSubsystem_K2_GetModCollectionInfoAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetModCollectionInfoAsync.png)

```cpp
void K2_GetModCollectionInfoAsync(FModioModCollectionID ModCollectionId, FOnGetModCollectionInfoDelegate Callback)
```
...
|`Target`|`ModioSubsystem`|
|`ModCollectionId`|Mod Collection ID of the mod collection to fetch data.|
|`Callback`|Callback providing a status code and an optional `FModioModCollectionInfo` object with the mod collection's extended information.|
</RefTable>
...
#### GetModCollectionMediaAsync (Avatar)
...
![nd_img_ModioSubsystem_K2_GetModCollectionAvatarAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetModCollectionAvatarAsync.png)

```cpp
void K2_GetModCollectionAvatarAsync(FModioModCollectionID ModCollectionId, EModioAvatarSize AvatarSize, FOnGetModCollectionMediaDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`ModCollectionId`|Mod Collection ID for use in creator avatar retrieval.|
|`AvatarSize`|Parameter indicating the size of avatar image that's required.|
|`Callback`|Callback providing a status code and an optional path object pointing to the location of the downloaded image|
</RefTable>
...
![nd_img_ModioSubsystem_K2_GetAvailableUserEntitlementsAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_GetAvailableUserEntitlementsAsync.png)

```cpp
void K2_GetAvailableUserEntitlementsAsync(FModioEntitlementParams Params, FOnGetAvailableUserEntitlementsDelegate Callback)
```
...
![nd_img_ModioSubsystem_K2_FollowUserAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_FollowUserAsync.png)

```cpp
void K2_FollowUserAsync(FModioUserID UserId, FOnErrorOnlyDelegate Callback)
```
...
|-|-|
|`Target`|`ModioSubsystem`|
|`UserId`|The UserID of the user that should be followed.|
|`Callback`|a callback invoked upon completion, indicating if the request was successful.|
</RefTable>
...
![nd_img_ModioSubsystem_K2_FollowModCollectionAsync.png](img/refdocs/nd_img_ModioSubsystem_K2_FollowModCollectionAsync.png)

```cpp
void K2_FollowModCollectionAsync(FModioModCollectionID ModCollectionToFollow, FOnFollowModCollectionDelegate Callback)
```
...
|`Callback`|Callback invoked when the follow request is completed.|
</RefTable>
...
![nd_img_ModioSubsystem_GetCurrentPortal.png](img/refdocs/nd_img_ModioSubsystem_GetCurrentPortal.png)
...
### ModioUserListLibrary 

Utility functions for accessing and managing lists of user information
...
### ModioEntitlementList 

Strong type struct to wrap multiple Entitlement indexed by a paged result
...
Information related to a SKU entitlement
...
| | | |
|-|-|-|
| `FString` | `SkuId` | Id of the Sku the entitlement is for |
| `EModioEntitlementType` | `EntitlementType` | The type of the entitlement |
</RefTable>
...
| `TArray<FModioPlacement>` | `Placements` | Placements available for this game |
| `EModioGameCloudCookingStatus` | `CloudCookingStatus` | Cloud Cooking status for this game |
</RefTable>
...
Placements of a game to represent dynamic mod view
...
| | | |
|-|-|-|
| `int64` | `ID` | Unique placement id |
| `EModioPlacementType` | `Type` | Placement type |
| `FString` | `Name` | Placement name |
| `FString` | `NameLocalized` | Placement name localized |
| `FModioFilterParams` | `Filters` | Structured filter describing the mods this placement should display |
| `int64` | `DisplayPosition` | Placement's display position |
| `int64` | `Total` | Total number in the placement to show |
| `EModioPlacementSize` | `Size` | Placement's display size |
| `bool` | `bEnabled` | Placement's enabled status |
</RefTable>
...
### ModioModCollectionInfoList 

Strong type struct to wrap multiple ModCollectionInfo indexed by a paged result
...
### ModioModCollectionInfo 

Information about a given mod collection
...
| | | |
|-|-|-|
| `FModioModCollectionID` | `Id` | The unique ID of the collection. |
| `FModioGameID` | `GameId` | The unique ID of the game the collection belongs to. |
| `FString` | `GameName` | the game name |
| `EModioModServerSideStatus` | `CollectionStatus` | The current ModStatus on the server: Accepted, NotAccepted, or Deleted. |
| `EModioObjectVisibilityFlags` | `Visibility` | The mod collection's visibility status. Defaults to Public (1) |
| `FModioUser` | `ProfileSubmittedBy` | Information on the user who submitted the collection |
| `FString` | `Category` | Chosen category of the collection |
...
| `bool` | `Incomplete` | boolean to indicate whether the collection contains hidden mods |
| `EModioMaturityFlags` | `ProfileMaturityOption` | Maturity options flagged by the mod developer. This is only relevant if the parent game allows collections to be labeled as mature. The value of this field will default to None unless the parent game allows to flag mature content. |
| `FModioUnsigned64` | `FileSize` | Size of collection in bytes |
| `FModioUnsigned64` | `FileSizeUncompressed` | Size of the uncompressed collection in bytes |
| `TArray<EModioModfilePlatform>` | `Platforms` | Supported platform for this collection |
| `TArray<FString>` | `Tags` | Tags this mod collection has set |
| `FModioModCollectionStats` | `Stats` | Stats and rating information for the mod collection. |
| `FModioLogo` | `Logo` | Media data related to the mod collection logo |
| `FString` | `ProfileName` | Name of the mod collection |
| `FString` | `ProfileNameId` | Name id of the mod collection |
| `FString` | `ProfileSummary` | Summary of the mod collection |
| `FString` | `ProfileDescription` | Detailed description in HTML format |
| `FString` | `ProfileDescriptionPlaintext` | Detailed description in plaintext format |
</RefTable>
...
| | | |
|-|-|-|
| `int64` | `DownloadsToday` | Number of mod collection downloads today. |
| `int64` | `DownloadsTotal` | Number of total mod collection downloads. |
| `int64` | `DownloadsUnique` | Number of unique mod collection downloads. |
| `int64` | `RatingTotal30Days` | Number of rating over 30 days. |
| `int64` | `RatingPositive30Days` | Number of positive ratings over 30 days. |
| `int64` | `RatingNegative30Days` | Number of negative ratings over 30 days. |
| `int64` | `RatingTotal` | Number of times this mod collection has been rated. |
| `int64` | `RatingPositive` | Number of positive ratings. |
| `int64` | `RatingNegative` | Number of negative ratings. |
| `int64` | `NumberOfComments` | Number of comments. |
| `int64` | `NumberOfFollowers` | Number of followers. |
| `int64` | `NumberOfMods` | Number of mods in the collection. |
</RefTable>
...
### ModioModCollectionID 

Strong type struct to wrap a ModCollectionID to uniquely identify collections of user-generated content.
...
| `TArray<FModioModMonetizationSKU>` | `SKUMappings` |  |
</RefTable>
...
### ModioUserModRatingList 

Strong type struct to wrap multiple user ratings indexed by a paged result
...
| `TArray<FModioUserModRating>` | `InternalList` | Internal array of user ratings |
</RefTable>
...
### ModioOptionalModCollectionInfo 

Struct to wrap ModCollectionInfo into an optional parameter
...
### ModioOptionalEntitlementList 

Struct to wrap EntitlementList into an optional parameter
...
### ModioOptionalUserModRatingList 

Struct to wrap UserModRatingList into an optional parameter
...
### ModioOptionalModCollectionInfoList 

Struct to wrap ModCollectionInfoList into an optional parameter
...
Strong type struct to wrap a ModChangeMap as an optional value
...
### ModioOptionalEntitlement 

Strong type struct to store an optional Entitlement parameter
...
### ModioServerInitializeOptions 

Struct type to store the initialization options used by the mod.io SDK in the context of a dedicated server
...
| | | |
|-|-|-|
| `FModioInitializeOptions` | `BaseOptions` | The base initialize options required in any Mod.io initialization context. |
| `FString` | `ModsDirectory` | The directory where mods should be stored on this Server |
| `FString` | `Token` | The S2S token for this Server |
| `TArray<FModioModID>` | `Mods` | What mods this Server will use. |
</RefTable>
...
Composite key for looking up storage metrics by location and usage type. * Combines an `EModioStorageLocation` (where the data lives) with an `EModioStorageUsage` (whether reporting consumed vs. available space). Implements equality and hashing so it can be used as a key in hashed containers (e.g., `TMap`).
...
Strongly-typed container for storage space metrics (consumed/available) across storage locations. * Stores storage values (in bytes) keyed by `EModioStorageLocation` and `EModioStorageUsage`, and provides convenience accessors via `GetSpace(...)` to query the recorded amount for a given key.
...
| `FModioLink` | `MonetizationLink` | Link to the mod.io Monetization Policy |
...
Strong type struct to store an optional TokenPack parameter
...
| `EModioRevenueFilterType` | `RevenueField` | Field to use for revenue (default is FreeAndPaid) |
| `TSoftClassPtr<UWidget> ` | `TileWidgetClass` | Field to use for the widget class (default is nullptr) |
...
```cpp
FText FileSizeUnsigned64_ToText(FModioUnsigned64 FileSize, int32 MinDecimals, int32 MaxDecimals, EFileSizeUnit Unit, bool bIncludeUnitName)
```
...
Retrieve the list of users from the user list

![nd_img_ModioUserListLibrary_GetUsers.png](img/refdocs/nd_img_ModioUserListLibrary_GetUsers.png)

```cpp
TArray<FModioUser> GetUsers(FModioUserList UserList)
```
...
| | |
|-|-|
|`UserList`|The user list to retrieve the Users from|
</RefTable>



#### Returns
An array of FModioUser objects representing the Users in the list
...
Retrieve paged result information for a user list

![nd_img_ModioUserListLibrary_GetPagedResult.png](img/refdocs/nd_img_ModioUserListLibrary_GetPagedResult.png)

```cpp
FModioPagedResult GetPagedResult(FModioUserList UserList)
```
...
| | |
|-|-|
|`UserList`|The user list to retrieve paged result information from|
</RefTable>
...
### Make Server Initialize Options

Make [`ModioServerInitializeOptions`](#modioserverinitializeoptions). Should only be used in conjunction with [`InitializeServerAsync`](#initializeserverasync).

![nd_img_ModioCommonTypesLibrary_MakeServerInitializeOptions.png](img/refdocs/nd_img_ModioCommonTypesLibrary_MakeServerInitializeOptions.png)

```cpp
FModioServerInitializeOptions MakeServerInitializeOptions(FModioInitializeOptions BaseOptions, FString ModsDirectory, FString Token, TArray<FModioModID> Mods)
```
...
|-|-|
|`BaseOptions`||
|`ModsDirectory`|The directory to store modfiles on this server.|
|`Token`|The S2S token for this Server.|
|`Mods`|The mods this server should load and use.|
</RefTable>
...
### Get Raw Value from Mod Collection ID

Retrieves the raw underlying value from an `ModioModCollectionID`. `ModioModCollectionID`s are intended as opaque types, so use with care.

![nd_img_ModioCommonTypesLibrary_GetRawValueFromModCollectionID.png](img/refdocs/nd_img_ModioCommonTypesLibrary_GetRawValueFromModCollectionID.png)

```cpp
int64 GetRawValueFromModCollectionID(FModioModCollectionID In)
```
...
| | |
|-|-|
|`In`|The `ModioModCollectionID` to retrieve the value for|
</RefTable>
...
Compares two mod Collection IDs to check whether they're equal

![nd_img_ModioCommonTypesLibrary_EqualToModCollectionID.png](img/refdocs/nd_img_ModioCommonTypesLibrary_EqualToModCollectionID.png)

```cpp
bool EqualToModCollectionID(FModioModCollectionID A, FModioModCollectionID B)
```
...
### Set Source File Version String

Sets the version string of the source file

![nd_img_ModioCreateModLibrary_SetSourceFileVersionString.png](img/refdocs/nd_img_ModioCreateModLibrary_SetSourceFileVersionString.png)

```cpp
void SetSourceFileVersionString(FModioCreateSourceFileParams In, FString Version)
```
...
| | |
|-|-|
|`In`|The source file creation parameters to modify|
|`Version`|The version string of the source file|
</RefTable>
...
### Set Source File Platforms

Sets the source file platforms of the source file

![nd_img_ModioCreateModLibrary_SetSourceFilePlatforms.png](img/refdocs/nd_img_ModioCreateModLibrary_SetSourceFilePlatforms.png)

```cpp
void SetSourceFilePlatforms(FModioCreateSourceFileParams In, TArray<EModioModfilePlatform> Platforms)
```
...
|-|-|
|`In`|The source file creation parameters to modify|
|`Platforms`|The source file platforms of the source file|
</RefTable>
...
### Set Source File Metadata Blob

Sets the source file metadata blob of the source file

![nd_img_ModioCreateModLibrary_SetSourceFileMetadataBlob.png](img/refdocs/nd_img_ModioCreateModLibrary_SetSourceFileMetadataBlob.png)

```cpp
void SetSourceFileMetadataBlob(FModioCreateSourceFileParams In, FString MetadataBlob)
```
...
|-|-|
|`In`|The source file creation parameters to modify|
|`MetadataBlob`|The source file metadata blob of the source file|
</RefTable>
...
### Set Source File Changelog String

Sets the changelog string of the source file

![nd_img_ModioCreateModLibrary_SetSourceFileChangelogString.png](img/refdocs/nd_img_ModioCreateModLibrary_SetSourceFileChangelogString.png)

```cpp
void SetSourceFileChangelogString(FModioCreateSourceFileParams In, FString Changelog)
```
...
|-|-|
|`In`|The source file creation parameters to modify|
|`Changelog`|The changelog string of the source file|
</RefTable>
...
Retrieve paged result information for a mod collection info list

![nd_img_ModioModCollectionInfoListLibrary_GetPagedResult.png](img/refdocs/nd_img_ModioModCollectionInfoListLibrary_GetPagedResult.png)

```cpp
FModioPagedResult GetPagedResult(FModioModCollectionInfoList ModCollectionInfoList)
```
...
| | |
|-|-|
|`ModCollectionInfoList`|The mod collection info list to retrieve paged result information from|
</RefTable>
...
Retrieve the list of mod collections from the mod collection info list

![nd_img_ModioModCollectionInfoListLibrary_GetCollections.png](img/refdocs/nd_img_ModioModCollectionInfoListLibrary_GetCollections.png)

```cpp
TArray<FModioModCollectionInfo> GetCollections(FModioModCollectionInfoList ModCollectionInfoList)
```
...
| | |
|-|-|
|`ModCollectionInfoList`|The mod collection info list to retrieve the mod collections from|
</RefTable>



#### Returns
An array of FModioModCollectionInfo objects representing the mod collections in the list
...
### BytesGreaterThan (Filesize)



![nd_img_ModioSDKLibrary_IsFilesizeGreaterThanUnit.png](img/refdocs/nd_img_ModioSDKLibrary_IsFilesizeGreaterThanUnit.png)

```cpp
bool IsFilesizeGreaterThanUnit(FModioUnsigned64 FileSize, EFileSizeUnit Unit)
```
...
Get the previous file size unit from the current unit.

![nd_img_ModioSDKLibrary_GetPreviousSize.png](img/refdocs/nd_img_ModioSDKLibrary_GetPreviousSize.png)

```cpp
EFileSizeUnit GetPreviousSize(EFileSizeUnit Unit, bool WrapAround)
```
...
| | |
|-|-|
|`Unit`|The current file size unit.|
|`WrapAround`|Whether to wrap around to the largest unit if the current unit is the smallest.|
</RefTable>



#### Returns
The previous file size unit.
...
Get the next file size unit from the current unit.

![nd_img_ModioSDKLibrary_GetNextSize.png](img/refdocs/nd_img_ModioSDKLibrary_GetNextSize.png)

```cpp
EFileSizeUnit GetNextSize(EFileSizeUnit Unit, bool WrapAround)
```
...
|-|-|
|`Unit`|The current file size unit.|
|`WrapAround`|Whether to wrap around to the smallest unit if the current unit is the largest.|
</RefTable>



#### Returns
The next file size unit.
...
```cpp
EFileSizeUnit GetDesiredFileSizeUnit_Unsigned64(FModioUnsigned64 FileSize)
```
...
### ToString (EFileSizeUnit)

Convert a file size unit to a human-readable string.

![nd_img_ModioSDKLibrary_FileSizeUnit_ToString.png](img/refdocs/nd_img_ModioSDKLibrary_FileSizeUnit_ToString.png)

```cpp
FString FileSizeUnit_ToString(EFileSizeUnit Unit)
```
...
| | |
|-|-|
|`Unit`|The file size unit to convert.|
</RefTable>



#### Returns
A formatted string representing the file size unit.
...
```cpp
FText Filesize_ToString_Unsigned64(FModioUnsigned64 FileSize, int32 MinDecimals, int32 MaxDecimals, EFileSizeUnit Unit, bool bIncludeUnitName)
```
...
Returns the byte size of the given FilesizeUnit (e.g EFileSizeUnit::KB returns 1024)

![nd_img_ModioSDKLibrary_Filesize_ToBytes.png](img/refdocs/nd_img_ModioSDKLibrary_Filesize_ToBytes.png)

```cpp
FModioUnsigned64 Filesize_ToBytes(EFileSizeUnit FileSizeUnit)
```
...
| | |
|-|-|
|`FileSizeUnit`|The unit to convert to a byte value|
</RefTable>



#### Returns
The byte value of the given unit.
...
|`Mod`|The ID of the mod you are submitting a file for|
|`Params`|Information about the mod file being created, including the memory that will be uploaded as a mod|
</RefTable>
...
### EModioGameCloudCookingStatus {#EModioGameCloudCookingStatus} 
...
### EModioPlacementSize {#EModioPlacementSize} 

Size of the placement to display
...
### EModioPlacementType {#EModioPlacementType} 
...
| `Switch2` |  |
| `Discord` |  |
| `PS` | Deprecated as of 2026.9 release. Please use PS |
| `Epic` |  |
| `Oculus` |  |
| `OpenID` |  |
| `GoogleIDToken` |  |
...
| `MonetizationPlatformAuthInvalid` | Platform auth failure when performing a monetization operation. The platform auth parameters may be invalid or expired. |
...
| `InvalidZipFile` | The zip file submitted is invalid. |
| `CloudCookingDisabled` | Cloud Cooking is disabled for this game. |
| `AlreadySubscribedToUser` | The authenticated user is already subscribed to the user. |
...
| `FailedStoreNotLoaded` | The current platform has not loaded the store skus yet * |
| `FailedStoreEmpty` | The current platform has no products in the store * |
...
Identifies the backing location used to store data.
...
Indicates how a storage amount should be interpreted when reporting disk usage. * Use `Consumed` for space already used by mod.io data, and `Available` for free space that can be used for downloads/installs.
...
Enum for the standard size a gallery image can use
...
| `FreeAndPaidWeighted` | Return both free and paid mods at game specified weighting |
</RefTable>

---

### EModioFilterTimespan {#EModioFilterTimespan} 

Enum indicating the duration from which results should be included
