...
- A mechanism in which you log in players to your service and then return an ID Token to [send to mod.io](https://docs.mod.io/#authentiate-via-openid).
...
- The stateless nature of ID Token authentication should not circumvent obtaining user consent from the player prior to sending ID Tokens to mod.io. See [Terms & User Consent](#user-consent).
...
- [REST API OpenID Endpoint](https://docs.mod.io/#authenticate-via-openid) - The API endpoint your game client (or web app) sends the ID Token to.
