...
custom_edit_url: https://github.com/modio/modio-ue4-internal/blob/develop/Plugins/Modio/Doc/getting-started/submit-mods.mdx
---
...
To submit a mod, you must first create a mod handle using [`GetModCreationHandle`](/unrealref/#K2_GetModCreationHandle), and use that handle when calling [`SubmitNewModAsync`](/unrealref/#K2_SubmitNewModAsync). Note that the newly created mod will remain hidden until a mod file is added in the next step.
...
Once you have successfully submitted a mod, you can submit a mod file for that mod using [`SubmitNewModFileForMod`](/unrealref/#K2_SubmitNewModFileForMod). When you submit a mod file, you pass a [`ModioCreateModFileParams`](/unrealref/#_modiocreatemodfileparams) containing the directory containing all the files that you want to submit. The plugin will compress this folder into a .zip file and upload it as the active version of the mod. 
