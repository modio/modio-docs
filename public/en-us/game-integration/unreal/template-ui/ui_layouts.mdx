...
custom_edit_url: https://github.com/modio/modio-ue4-internal/tree/main/Plugins/ModioComponentUI/Doc/template-ui/ui_layouts.mdx
---
...
### `WBP_ModTileView`
The mod tile that will populate the browser. Their scale and padding in the browser can be set via the `WBP_ModTileView` layout in the `WBP_ModioModBrowser` layout.

![modio mod tile](img/modtileview.png)

### `WBP_ModModDetailsDialog`
...
Inside `WBP_ModioModBrowser`, you can find a modified grid layout called `WBP_ModTileView`. This is populated with `WBP_ModioDefaultModTile`. The `WBP_ModTileView` layout controls the size and padding of the tiles.
